...
**배포 서버에서는 포트를 여는 주소를 좁힙니다.** 위 포트는 기본으로 모든 주소(`0.0.0.0`)에 열립니다. 로컬 개발은 그대로 두면 되고, 배포 서버에서는 `.env` 의 두 값으로 여는 주소를 바꿉니다.

| 변수 | 비워 둘 때 (로컬) | 배포 서버 |
|---|---|---|
| `PUBLISH_ADDR` | `0.0.0.0` — 모든 주소 | `127.0.0.1` — 서버 안에서만 보임 |
| `GATEWAY_ADDR` | `0.0.0.0` — 모든 주소 | `10.8.0.2` — 앞단 nginx 가 WireGuard 터널로 찾아오는 주소 |

`/internal` 경로에는 인증을 붙이지 않았으므로, 배포 서버에서는 게이트웨이 말고는 바깥에 열지 않습니다. `PUBLISH_ADDR` 은 게이트웨이를 뺀 포트 24줄에, `GATEWAY_ADDR` 은 게이트웨이 한 줄에 쓰입니다.

---

**게이트웨이를 터널 주소에 묶으면 부팅 순서를 정해야 합니다.** 도커가 터널보다 먼저 뜨면 `10.8.0.2` 가 아직 없어서 게이트웨이 컨테이너가 포트를 잡지 못합니다. 배포 서버에 아래 파일을 두면 도커가 터널 다음에 시작합니다.

```ini
# /etc/systemd/system/docker.service.d/after-wireguard.conf
[Unit]
